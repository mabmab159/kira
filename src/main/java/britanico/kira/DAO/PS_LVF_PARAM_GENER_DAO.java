...
public interface PS_LVF_PARAM_GENER_DAO extends JpaRepository<PS_LVF_PARAM_GENER, PS_LVF_PARAM_GENER_ID> {
    ...
    @Query(value = "SELECT (CONVERT(CHAR(10), X.LAST_ENRL_DT_STMP, 121)), X.FEC_FIN_CL, YEAR(X.FEC_FIN_CL) AS YEAR, " +
            "MONTH(X.FEC_FIN_CL) AS MES , DATEADD(MONTH, ((YEAR(X.END_DT) - 1900) * 12) + MONTH(X.END_DT) + CAST" +
            "(:meses_discontinuidad AS INT) +1, -1) FROM (SELECT IC.EMPLID, IC.ACAD_CAREER, IC.STRM, IC.CLASS_NBR, IC" +
            ...
            ".SESSION_CODE AND CS.CLASS_NBR = IC.CLASS_NBR INNER JOIN (SELECT CRSE_ID, CRSE_OFFER_NBR, STRM, " +
            "SESSION_CODE, CLASS_SECTION, MAX(END_DT) AS FEC_FIN_CL FROM PS_CLASS_MTG_PAT GROUP BY CRSE_ID, " +
            "CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION) RE ON RE.CRSE_ID = CS.CRSE_ID AND RE.CRSE_OFFER_NBR =" +
            " CS.CRSE_OFFER_NBR AND RE.STRM = CS.STRM AND RE.SESSION_CODE = CS.SESSION_CODE AND RE.CLASS_SECTION = CS" +
            ".CLASS_SECTION LEFT JOIN PS_GRADE_TBL GB ON GB.SETID=IC.INSTITUTION AND GB.GRADING_SCHEME=IC" +
            ".GRADING_SCHEME_ENR AND GB.GRADING_BASIS=IC.GRADING_BASIS_ENRL AND GB.CRSE_GRADE_INPUT=IC.CRSE_GRADE_OFF" +
            " AND GB.EFFDT = (SELECT MAX(B_ED.EFFDT) FROM PS_GRADE_TBL B_ED WHERE GB.SETID = B_ED.SETID AND GB" +
            ".GRADING_SCHEME = B_ED.GRADING_SCHEME AND B_ED.EFFDT <= GETDATE()) WHERE IC.INSTITUTION = :institution " +
            "AND IC.EMPLID = :emplid AND IC.STDNT_ENRL_STATUS = 'E' AND IC.CRSE_GRADE_OFF <> ' ' AND IC" +
            ".GRADING_BASIS_ENRL IN ('ANC','GRD','GRE','GRP') AND GB.DESCR <> 'FDA' AND GB.DESCR <> 'FAIL DUE TO " +
            "ABSENCES' AND CASE WHEN (DATEADD(MONTH, ((YEAR(CS.END_DT) - 1900) * 12) + MONTH(CS.END_DT) + CAST" +
            "(:meses_discontinuidad AS INT) +1, -1))> GETDATE() THEN 1 ELSE 0 END=1) X WHERE X.NUM = 1", nativeQuery
            = true)
    public String lastCourseNoFDA(@Param("institution") String institution, @Param("emplid") String emplid,
