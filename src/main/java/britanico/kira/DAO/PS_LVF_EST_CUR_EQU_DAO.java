...
public interface PS_LVF_EST_CUR_EQU_DAO extends JpaRepository<PS_LVF_EST_CUR_EQU, PS_LVF_EST_CUR_EQU_ID> {
    ...
    @Query(value = "select Distinct B.CRSE_ID as claseAnterior, A.CRSE_ID as claseSiguiente, A.ACAD_GROUP from " +
            "PS_CRSE_OFFER A inner join PS_RQ_GRP_DETL_TBL B on A.RQRMNT_GROUP=B.RQRMNT_GROUP where B.CRSE_ID<>'' and " +
            "B.CRSE_ID = :sesionAnterior order by A.ACAD_GROUP desc", nativeQuery = true)
    List<Map<String, String>> siguienteCurso(@Param("sesionAnterior") String sesionAnterior);
